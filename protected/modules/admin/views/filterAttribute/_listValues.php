    <?php Yii::app()->clientScript->registerScript('add-list', "
		...
		$('#list-button').on('click', function() {
			...
			$.post($(this).data('url'), {'FilterAttributeListValue[title]' : title}, function(json) {
                if(json.success) {
...
<div id="list-form" style="<?php echo $model->type === Attribute::TYPE_LIST ? '' : 'display:none' ?>" >
    ...
    <?php $this->widget('bootstrap.widgets.TbButton', array(
		...
		'htmlOptions'=>array(
			...
			'data-url' => $this->createUrl('/admin/filterAttributeListValue/create')
		)
    )); ?>
	...
	<div id="list-grid" class="grid-view" style="width: 660px;">
		...
		<?php
			foreach($model->listValues as $row) {
				$this->renderPartial('modules.admin.views.filterAttributeListValue._view', array(
					'model' => $row
				));
			}
		?>
