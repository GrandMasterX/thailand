    <?php Yii::app()->clientScript->registerScript('add-list', "
		...
		$('#type').on('change', function() {
			$(this).val() == '" . Attribute::TYPE_LIST . "' ? $('#list-form').show() : $('#list-form').hide();
		})
		...
		$('#list-button').on('click', function() {
			...
			$.post($(this).data('url'), {'AttributeList[title]' : title}, function(json) {
                if(json.success) {
...
<div id="list-form" style="<?php echo $model->type === Attribute::TYPE_LIST ? '' : 'display:none' ?>" >
    ...
    <?php $this->widget('bootstrap.widgets.TbButton', array(
		...
		'htmlOptions'=>array(
			...
			'data-url' => $this->createUrl('/admin/attributeList/create')
		)
    )); ?>
	...
	<div id="list-grid" class="grid-view" style="width: 660px;">
		...
		<?php
			foreach($model->list as $row) {
				$this->renderPartial('modules.admin.views.attributeList._view', array(
					'model' => $row
				));
			}
		?>
