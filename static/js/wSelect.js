(function($) {
    ...
    Select.prototype = {
        generate: function() {
            if (!this.$select) {
                var _self = this;
                var _selectCSSClass = this.$el[0].className;
		    if(_selectCSSClass === undefined){
			    _selectCSSClass = '';
		    }

                this.$select = $('<div class="wSelect '+_selectCSSClass+'"><div class="wSelect-arrow"></div></div>');
        ...
        keydown: function(e) {
            // tab
            if (e.keyCode === 9) {
                if(this.$select.hasClass('opened')){
        ...
        onBlur: function(className) {
            className = className || 'active';
		if(this.$select.hasClass('opened') && !this.$select.hasClass('wSelect-hover')){
			this.$select.removeClass('opened');
		}
            if (this.options.highlight) {
    ...
    Option.prototype = {
        ...
        select: function() {
		    ...
		    if(this.wSelect.$select.hasClass('opened')){
			    this.wSelect.$select.removeClass('opened');
		    }
            }
        ...
        setWidth: function() {
            ...
            if ((optionWidth > this.wSelect.$select.width()) && this.changeWidth) {
